...
if d == 10:
    ...
    noise_stddevs = [np.exp(s*np.log(10.0)) for s in np.linspace(0,-2,50)]
    ...
    for maxiter in L_maxiter:
        for lbfgs_rank in L_lbfgs_rank:
            for act_func in L_act_func:
                ...
                output_dir = "/data/lisatmp2/alaingui/dae/dae_trained_models/gaussian_mixture_d%d/experiment_07/%0.6d" % (d, output_dir_counter)
