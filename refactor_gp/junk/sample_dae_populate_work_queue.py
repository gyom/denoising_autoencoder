...
L_langevin_beta = [2.0, 1.0, 1.0, 0.5, 0.5]
...
experiment_name = "experiment_28_10"
                    ...
                    for langevin_stddev in L_langevin_stddev:
                        for langevin_beta in L_langevin_beta:
                            for n_E_approx_path in L_n_E_approx_path:
                                ...
                                params = (script_path, pickled_dae_dir, n_samples, thinning_factor, n_E_approx_path, langevin_stddev, langevin_beta, want_overview_plots, mcmc_method, omit_asymmetric_proposal_factor, proposal_noise_scheme, output_dir)
                                command = """python %s --pickled_dae_dir=%s --n_samples=%d --thinning_factor=%d --n_E_approx_path=%d --langevin_stddev=%f --langevin_beta=%f --want_overview_plots=%s --mcmc_method=%s --omit_asymmetric_proposal_factor=%d --proposal_noise_scheme=%s --output_dir=%s""" % params

                                print command
                                r_server.rpush("train_gaussian_mixture", command)

                elif mcmc_method in ["MH_grad_E"]:
                    for proposal_stddev in L_proposal_stddev:
                        for n_E_approx_path in L_n_E_approx_path:
                            output_dir = "/data/lisatmp2/alaingui/dae/generated_samples/%s/%0.6d" % (experiment_name, output_dir_counter)
                            output_dir_counter += 1

                            params = (script_path, pickled_dae_dir, n_samples, thinning_factor, n_E_approx_path, proposal_stddev, want_overview_plots, mcmc_method, omit_asymmetric_proposal_factor, proposal_noise_scheme, output_dir)
                            command = """python %s --pickled_dae_dir=%s --n_samples=%d --thinning_factor=%d --n_E_approx_path=%d --proposal_stddev=%f --want_overview_plots=%s --mcmc_method=%s --omit_asymmetric_proposal_factor=%d --proposal_noise_scheme=%s --output_dir=%s""" % params
